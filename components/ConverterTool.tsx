    ...
    <section id="tool" className="scroll-mt-20 pb-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* LEFT — Input */}
          <div className="bg-white border border-hairline rounded-lg shadow-sm self-start">
            ...
            <div className="p-5">
              {inputMode === "file" ? (
                <div ref={dragRef}
                  ...
                  className="border-2 border-dashed border-hairline rounded-lg p-8 text-center cursor-pointer hover:border-cobalt/40 transition-colors">
                  ...
                  <Upload className="size-8 mx-auto mb-3 text-ink/30" />
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 justify-center">
                      {files.map((f, i) => (
                        <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-paper-alt text-xs font-medium text-ink">
                          {f.name}
                          <button onClick={(e) => { e.stopPropagation(); setFiles((prev) => prev.filter((_, idx) => idx !== i)) }} className="text-ink/40 hover:text-forest ml-1">&times;</button>
                        </span>
                      ))}
                    </div>
                  )}
              ...
              <div className="mt-4 flex items-center gap-3">
                <button onClick={handleConvert} disabled={loading}
                  className="px-6 py-2.5 bg-ink text-paper text-sm font-medium rounded-lg hover:brightness-110 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex items-center gap-2 shadow-sm">
                  {loading ? t("tool.converting") : t("tool.convert")} {!loading && <ArrowRight className="size-4" />}
                </button>
                <span className="text-xs text-ink/30 font-mono">{t("tool.shortcut")}</span>
              </div>
            ...
            {history.length > 0 && (
              <div className="border-t border-hairline">
                <button onClick={() => setShowHistory(!showHistory)}
                  className="flex items-center justify-between w-full px-5 py-2.5 text-xs font-medium text-ink/50 hover:text-ink transition-colors">
                  ...
                  <div className="px-3 pb-3 space-y-1 max-h-48 overflow-auto">
                    {history.map((item) => (
                      <button key={item.id} onClick={() => loadFromHistory(item)}
                        className="w-full text-left px-3 py-2 rounded-md text-xs text-ink/60 hover:bg-paper-alt hover:text-ink transition-colors flex items-center justify-between">
                        <span className="truncate mr-2">{item.filename}</span>
                        <span className="text-ink/30 shrink-0">{item.words} words</span>
                      </button>
                    ))}
                    <button onClick={clearHistory} className="flex items-center gap-1 text-xs text-ink/30 hover:text-forest transition-colors pt-1 px-1">
          ...
          {/* RIGHT — Preview */}
          <div className="bg-white border border-hairline rounded-lg shadow-sm min-h-[350px] flex flex-col">
            {result ? (
              <>
                <div className="flex items-center justify-between px-5 py-3 border-b border-hairline bg-paper-alt/50">
              ...
              <div className="flex-1 flex items-center justify-center text-sm text-ink/30 font-mono p-8 text-center">
                <div>
                  <Hash className="size-10 mx-auto mb-3 text-hairline" />
                  {t("tool.empty")}
                </div>
              </div>
            )}
