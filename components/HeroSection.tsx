    ...
    <section className="pt-10 pb-12 md:pt-16 md:pb-16">
      <div className="section-container">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {formats.map((f) => (
            <span key={f} className="font-mono text-[11px] px-2.5 py-1 rounded-full border border-hairline text-ink/50">{f}</span>
          ))}
        </div>

        <p className="mt-4 font-mono text-xs text-ink/40">{t("hero.stat")}</p>

        <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-0 border border-hairline rounded-lg overflow-hidden bg-white shadow-sm">
          ...
          <div className="hidden md:block absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
            <ArrowRight className="size-6 text-hairline" />
          </div>
        </div>
